...
function findPinkElephantsByTimestreet(songs) {
  const specificSong = songs.find(song => song.title === 'Pink Elephants' && song.artist === "Timestreet") 
  return specificSong;
}
...
function findFirstSongUnderThreeMinutes(songs) {
 return songs.find(song => song.runtimeInSeconds < 180) || null;
}
...
function findFirstTitleTrack(songs) {
  return songs.find(song => song.title === song.album) || null;
}
